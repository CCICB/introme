...
input="$prefix.spliceai.vcf"
output="$prefix.spliceai.rmcommas.vcf"

cp annotation_header.vcf introme_annotate.spliceai.vcf
bcftools view -h $input > $output
bcftools view -H $input | awk '$8 !~ /,/' >> $output 
...
while read line; do
    ...
    if [[ $scores_gene1 == $scores_gene2 ]]; then
        ...
        echo "$start"$'\t'"$spliceai_scores"$'\t'"$end" >> $output
...
done < <(bcftools view -H $input | awk '$8 ~ /,/')

# Format SpliceAI output for vcfanno
bcftools view -H $output | cut -f1-8 | grep "SpliceAI" | sed 's/|/gene=/' | sed 's/SpliceAI=*.*gene/gene/' | sed 's/|/;DS_AG=/' | sed 's/|/;DS_AL=/' | sed 's/|/;DS_DG=/' | sed 's/|/;DS_DL=/' | sed 's/|/;DP_AG=/' | sed 's/|/;DP_AL=/' | sed 's/|/;DP_DG=/' | sed 's/|/;DP_DL=/' >> introme_annotate.spliceai.vcf
