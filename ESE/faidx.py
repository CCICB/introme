...
def faidx_context(ref_genome: pysam.FastaFile, chrom: str, pos: int,
    ...
    try:
        assert(ref_allele == sequence[context_len:context_len + ref_len])
    except AssertionError:
        print(f"reference allele {ref_allele} did not match reference genome "
...
if __name__ == "__main__":
    ...
    def test_ref_and_alt(chrom: str, pos: int, ref_allele: str, alt_allele: str, expected: tuple):
        a = ref_and_alt(ref_genome, chrom, pos, ref_allele, alt_allele, CONTEXT_LENGTH, prepend_chr=True)
        try:
            assert(a == expected)
        except AssertionError:
